...
int main(int NbParam, char *Param[])
{
	...
	LeProb.Fonction = MAXSAT;				//**Spécifie le problème traité
...
void InitialisationIntervalleVariable(tProblem &unProb)
{
	...
	case MAXSAT:  	unProb.Xmin = 0; 	unProb.Xmax = 1.0;				  break;
...
void EvaluationPosition(tPosition &Pos, tProblem unProb, tPSO &unPSO)
{
	...
	case BANANE: //Rosenbrock ou fonction Banane. Min 0 en (1, ... 1)
		...
		break;
	case MAXSAT:
		for (auto clause : unProb.Clause)
		{
			bool clauseValeur = false;
			for (auto litteral : clause.Litt) {
				bool v = Pos.X[abs(litteral) - 1] > ((unProb.Xmax - unProb.Xmin) / 2);
				clauseValeur = clauseValeur || (litteral > 0 ? v : !v);
			}
			if (clauseValeur) {
				valeur += clause.Poids;
			}
			//cout << clauseValeur << endl;
		}
		//cout << "---" << endl;
		break;
...
void DeplacerUneParticule(tParticule &Particule, tProblem unProb, tPSO &unPSO)
{
	...
	{
		Particule.V[d] = max(Particule.V[d], -unPSO.Vmax);
		Particule.V[d] = min(Particule.V[d], +unPSO.Vmax);
	}

	//Mise à jour de la nouvelle position--------------------------------
	for (d = 0; d < unProb.D; d++) {
		double sig = 1.0 / (1.0 + exp(-Particule.V[d]));
		double rd = AleaDouble(0, 1);
		//cout << rd << " " << sig <<" "<< (rd < sig) << endl;
		Particule.Pos.X[d] =  (rd < sig) ? 1 : 0;
	}
